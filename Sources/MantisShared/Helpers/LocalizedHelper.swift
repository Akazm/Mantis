...
struct LocalizedHelper {
    ...
    static func getString(
        _ key: String,
        localizationConfig: LocalizationConfig = MantisShared.localizationConfig,
        value: String? = nil
    ) -> String {
        let value = value ?? key
...
#else
        guard let bundle = LocalizedHelper.bundle ?? (localizationConfig.bundle ?? MantisShared.bundle) else {
            return value
        }
